...
class Bot {
  ...
  sendToIRC(message) {
    ...
    //this does not provide accurate channel mapping debugging information because it excludes "message.channel.id" mapping
  ...
  sendToDiscord(author, channel, text) {
    ...
    // Don't pass on username-less commands from irc->discord if the formatting
    // setting is set to false. Useful for discord chans with no bots.
    if (this.isCommandMessage(text) && this.discordCommandFormatting) {
      patternMap.side = 'IRC';
